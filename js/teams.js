async function fetchContructorsList() {
    const apiUrl = 'https://ergast.com/api/f1/2024/constructors.json';
    let attempts = 3;
...
function displayContructorsList(data) {
    const contructors = data.MRData.ConstructorTable.Constructors;
    const container = document.createElement('div');
    container.className = 'space-y-4';

    contructors.forEach(contructor => {
        ...
        const contructorLink = document.createElement('a');
        contructorLink.href = `${contructor.url}`;
        contructorLink.appendChild(card);

        card.innerHTML = `<div class="flex-grow p-4 z-10">
            <h3 class="text-xl font-bold text-yellow-300">${contructor.name}</h3>
            <p class="text-gray-400">${contructor.nationality}</p>
        </div>
        ...
        container.appendChild(contructorLink);
    });
...
document.addEventListener('DOMContentLoaded', fetchContructorsList);